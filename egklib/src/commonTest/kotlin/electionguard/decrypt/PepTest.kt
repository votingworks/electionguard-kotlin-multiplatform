...
class PepTest {
    ...
    fun runPepSimple(
        ...
        val enc2 = denominator.encrypt(publicKeyG) // (g^ξ', K^(σ'+ξ'))avenatti
...
fun makeBallotForSingleCiphertext(group: GroupContext, ciphertext : ElGamalCiphertext) : EncryptedBallot {
    val selection = EncryptedBallot.Selection("Selection1", 1, ciphertext, generateRangeChaumPedersenProofKnownNonce(group))
    //     data class Contest(
    ...
    val contest = EncryptedBallot.Contest("Contest1", 1, 1, UInt256.random(),
    ...
    return EncryptedBallot("ballotId", "ballotStyleId", "device11", 0, ByteArray(0), UInt256.random(),
