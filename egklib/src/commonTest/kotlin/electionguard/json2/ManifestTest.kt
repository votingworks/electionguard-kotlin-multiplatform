    ...
    fun testManifestInputBuilderRoundtrip() {
        runTest {
            ...
            val manifest: Manifest = ebuilder.addContest("onlyContest")
                ...
                .done()
                .build()
            assertEquals(manifest, manifest.publishJson().import())
            assertEquals(manifest, jsonRoundTrip(manifest.publishJson()).import())
        }
    }
    ...
    fun testBallotStyle() {
        runTest {
            checkAll(
                ...
                assertEquals(obj, obj.publishJson().import())
    ...
    fun testManifest() {
        runTest {
            checkAll(
                ...
                val json = obj.publishJson()
                val rtProto = json.import()
