        ...
        runTest {
            ...
            val context =
                ElectionContext(
                    ...
                    electionRecord.manifest.cryptoHash,
                    group.TWO_MOD_Q.toUInt256(),
                    group.TWO_MOD_Q.toUInt256(),
                    group.TWO_MOD_Q.toUInt256(),
                    null
                )
            ...
            for (contest in result.contests) {
                // println(" contest ${contest.contestId} = ${contest.cryptoHash} nonce
                // ${contest.contestNonce}")
                for (selection in contest.selections) {
                    // println("  selection ${selection.selectionId} = ${selection.cryptoHash} nonce
