    ...
    private class BallotFactory {
        ...
        fun get_random_contest_from(contest: Manifest.ContestDescription): PlaintextBallot.Contest {
            ...
            for (selection_description in contest.selections) {
                val selection: PlaintextBallot.Selection =
                    get_random_selection_from(selection_description)
                voted += selection.vote
                if (voted <= contest.votesAllowed) {
                    selections.add(selection)
                }
            }
            return PlaintextBallot.Contest(contest.contestId, contest.sequenceOrder, selections)
        }

        companion object {
            fun get_random_selection_from(
                description: Manifest.SelectionDescription
