...
class ManifestHashValidateTest {
    ...
    fun showManifestHash(manifest: Manifest) {
        ...
        manifest.ballotStyles
            .forEach {
                val cryptoHash: UInt256 =
                    hashElements(it.ballotStyleId, it.geopoliticalUnitIds, it.partyIds, it.imageUri)
        ...
        manifest.candidates
            .forEach {
                val cryptoHash: UInt256 =
                    hashElements(it.candidateId, it.name, it.partyId, it.imageUri)
                println("  '$it' = $cryptoHash")
                println()
            }
        ...
        val expect =
            "b2af12d76e8d1a869213fa7f3136eede0bb125250b9f3a23a95998665180d45f".fromSafeHex()
                .toUInt256()
    ...
    fun validateManifestHash(manifest: Manifest) {
        ...
        for (contest in manifest.contests) {
            ...
            for (selection in contest.selections) {
                val scryptoHash =
                    selectionDescriptionCryptoHash(
                        selection.selectionId,
                        selection.sequenceOrder,
                        selection.candidateId,
                    )
                // println("    Selection ${selection.selectionId} crypto_hash:
