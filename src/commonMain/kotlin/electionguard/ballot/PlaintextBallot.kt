 ...
 * The object_id is a unique Ballot ID created by the external system. This is used both as input,
...
data class PlaintextBallot(
     ...
     * A complete representation of a ballot must include both affirmative and negative selections
     * of the contest, AND the placeholder selections necessary to satisfy the ConstantChaumPedersen
     * proof in the CiphertextBallotContest.
     *
     * Typically partial contests are passed into Electionguard for memory constrained systems,
     * while complete contests are passed into ElectionGuard when running encryption on an existing
     * dataset.
     */
    data class Contest(
     ...
     *
     * This can also be designated as `is_placeholder_selection` which has no context to the data
     * specification but is useful for running validity checks internally
     *
     * An `extended_data` field exists to support any arbitrary data to be associated with the
     * selection. In practice, this field is the cleartext representation of a write-in candidate
     * value. LOOK In the current implementation these write-in are discarded when encrypting.
     */
