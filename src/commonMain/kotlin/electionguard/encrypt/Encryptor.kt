 ...
 * them to this class.
 */
class Encryptor(val group: GroupContext, val manifest: Manifest, val context : ElectionContext,) {
    ...
    fun encrypt(
        ...
        for (ballot in ballots) {
            val encryptedBallot =
                ballot.encryptBallot(previousTrackingHash, group.randomElementModQ())
    ...
    fun PlaintextBallot.encryptBallot(
        ...
        val ballotNonce: UInt256 =
            hashElements(manifest.cryptoHashUInt256(), this.ballotId, randomMasterNonce)
        ...
        for (mcontest in manifest.contests) {
            val pcontest : PlaintextBallot.Contest =
                pcontests[mcontest.contestId]
    ...
    fun contestFrom(mcontest: Manifest.ContestDescription): PlaintextBallot.Contest {
        val selections =
            mcontest.selections
                .map { selectionFrom(it.selectionId, it.sequenceOrder, false, false) }
    ...
    fun PlaintextBallot.Contest.encryptContest(
        ...
        for (mselection: Manifest.SelectionDescription in contestDescription.selections) {
            ...
            encrypted_selection =
                plaintextSelection.encryptSelection(mselection, contestNonce, false,)
        ...
        if (validate &&
            !proof.isValid(ciphertextAccumulation, elgamalPublicKey, cryptoExtendedBaseHashQ, limit)
        ) {
            logger.warn { "Ballot $ballotId contest $contestId proof does not validate" }
        }

        val encrypted_contest: CiphertextBallot.Contest =
            CiphertextBallot.Contest(
                this.contestId,
                this.sequenceOrder,
    ...
    fun PlaintextBallot.Selection.encryptSelection(
        ...
        val nonceSequence =
            Nonces(selectionDescription.cryptoHash.toElementModQ(group), contestNonce)
        ...
        val elgamalEncryption: ElGamalCiphertext =
            this.vote.encrypt(elgamalPublicKey, selectionNonce)

        val proof =
            elgamalEncryption.disjunctiveChaumPedersenProofKnownNonce(
                this.vote,
                selectionNonce,
                elgamalPublicKey,
        ...
        if (validate &&
            !proof.isValid(elgamalEncryption, elgamalPublicKey, cryptoExtendedBaseHashQ,)
        ) {
        ...
        val encryptedSelection: CiphertextBallot.Selection =
            CiphertextBallot.Selection(
                this.selectionId,
                this.sequenceOrder,
