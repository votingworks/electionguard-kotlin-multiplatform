...
class DecryptingMediator(
    ...
    fun CiphertextTally.decrypt() : PlaintextTally {
        ...
        val tallySharesBySelectionId : MutableMap<String, MutableList<DecryptionShareSelection>> =
            HashMap()
        for (tallyShare in tallyShares) {
            for (tallyContest in tallyShare.contests) {
                for (selection in tallyContest.selections) {
                    ...
                    var slist : MutableList<DecryptionShareSelection>? =
                        tallySharesBySelectionId.get(hashId)
                    if (slist == null) {
                        slist = ArrayList()
    ...
    /**
     * Compute a guardian's share of a decryption for the tally, aka a 'partial decyrption'. <p>
     *
    ...
    fun CiphertextTally.computeDecryptionShareForTally(
        ...
        return DecryptionShare(guardian.id(), guardian.electionPublicKey(), contests)
    }
    ...
    fun computeAvailableGuardians() : List<AvailableGuardian> {
        ...
        for (decryptingTrustee in decryptingTrustees) {
            val seq_orders: List<Int> =
                decryptingTrustees.filter { !it.id().equals(decryptingTrustee.id()) }
                    .map { it.xCoordinate() }
            val coeff: Int = computeLagrangeCoefficient(decryptingTrustee.xCoordinate(), seq_orders)
            result.add(
                AvailableGuardian(decryptingTrustee.id(), decryptingTrustee.xCoordinate(), coeff)
            )
        }
        return result
    }
