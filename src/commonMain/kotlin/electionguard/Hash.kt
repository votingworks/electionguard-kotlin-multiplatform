...
fun GroupContext.hashElements(vararg elements: Any?): ElementModQ =
    hashElementsNoFormula(*elements)
    ...
    val hashMe = elements
        .map {
            when (it) {
                ...
                is Iterable<*> ->
                    // The simplest way to deal with lists and such are to crunch them recursively.
                    // But we special-case the empty list, because it hashes to "null" yet has a
                    // formula of []. Note that empty lists and nulls will never occur in practice,
                    // anywhere in ElectionGuard, but hashElements needs to handle them correctly,
                    // just in case.
                    if (it.none())
                        "null"
                    else
