...
class PlaintextBallotConvert {
    ...
    private fun convertContest(
        contest: electionguard.protogen.PlaintextBallotContest
    ...
    private fun convertSelection(
        selection: electionguard.protogen.PlaintextBallotSelection
    ...
    private fun convertExtendedData(
        data: electionguard.protogen.ExtendedData?
    ): PlaintextBallot.ExtendedData? {
        if (data == null) {
            return null
        }
        return PlaintextBallot.ExtendedData(data.value, data.length)
    }
    ...
    private fun convertExtendedData(
        data: PlaintextBallot.ExtendedData?
    ): electionguard.protogen.ExtendedData? {
        if (data == null) {
            return null
        }
        return electionguard.protogen.ExtendedData(data.value, data.length)
    }
}