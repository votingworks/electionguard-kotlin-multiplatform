...
data class ManifestToProto(val groupContext: GroupContext) {
    ...
    fun translateToProto(manifest: Manifest): electionguard.protogen.Manifest {
        return electionguard.protogen.Manifest(
            ...
            if (manifest.name == null) { null } else { convertInternationalizedText(manifest.name) },
            convertContactInformation(manifest.contactInformation),
        )
    }

    private fun convertAnnotatedString(annotated: Manifest.AnnotatedString ): electionguard.protogen.AnnotatedString{
    ...
    private fun convertContactInformation(contact: Manifest.ContactInformation?): electionguard.protogen.ContactInformation? {
        if (contact == null) {
            return null
        }
    ...
    private fun convertInternationalizedText(itext: Manifest.InternationalizedText?): electionguard.protogen.InternationalizedText? {
        if (itext == null) {
            return null
        }
