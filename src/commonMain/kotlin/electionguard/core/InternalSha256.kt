...
/**
 * This is a "pure Kotlin" implementation of HMAC-SHA256. It's useful for testing, and if we
 * really have to, we can use it in production, but it's not going to be as fast as a tuned
 * implementation. That's in `SHA256.kt`, which wraps JVM, native, and maybe JS implementations,
...
/**
 * This is a "pure Kotlin" implementation of SHA256. It's useful for testing, and if we
 * really have to, we can use it in production, but it's not going to be as fast as a tuned
 * implementation. That's in `SHA256.kt`, which wraps JVM, native, and maybe JS implementations,
 * different on each platform.
 */
